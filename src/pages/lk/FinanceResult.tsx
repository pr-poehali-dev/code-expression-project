...
function AiFinanceBlock({ data }: { data: FinanceData }) {
  const [status, setStatus] = useState<"idle" | "loading" | "done" | "error">("idle");
  const [sections, setSections] = useState<AiSection[]>([]);

  const r = calcAll(data);
  const label = getIFRLabel(r.ifr).label;

  const mindsetLabels: Record<string, string> = {
    fearRaisePrice:   "боюсь поднять цены",
    feelUnworthy:     "чувствую себя недостаточно ценным",
    fearLoseClients:  "боюсь потерять клиентов",
    hardToTalkMoney:  "сложно говорить о деньгах",
    incomeCapInHead:  "есть психологический потолок дохода",
  };
  const mindsetBlocks = Object.entries(data.mindset)
    .filter(([, v]) => v)
    .map(([k]) => mindsetLabels[k] || k);
  ...
  useEffect(() => {
    ...
    fetch(func2url["ai-finance"], {
      ...
      body: JSON.stringify({
        ...
        ifp: r.ifp,
        label,
        current_income: data.currentModel.currentIncome,
        desired_income: data.goals.desiredIncome,
        gap: r.fr,
        mpd: r.mpd,
        avg_check: data.currentModel.avgCheck,
        nsc: r.nsc,
        nck: r.nck,
        clients_per_month: data.currentModel.clientsPerMonth,
        hours_per_week: data.currentModel.hoursPerWeek,
        clean_profit: r.cp,
        ceiling_reached: r.ceilingReached,
        goal_description: data.goals.goalDescription,
        goal_months: data.goals.goalMonths,
        mindset_blocks: mindsetBlocks,
      }),
    })
      .then(res => res.json())
      .then(res => {
        if (res.sections) {
          const parsed: AiSection[] = Object.entries(res.sections).map(([title, content]) => ({
            title,
  ...
  const sectionIcons: Record<string, string> = {
    "ТВО ФИНАНСОВАЯ КАРТИНА": "BarChart2",
    "ГДЕ ЗАСТРЯЛИ ДЕНЬГИ": "AlertTriangle",
    "3 ДЕЙСТВИЯ НА ЭТОЙ НЕДЕЛЕ": "ListChecks",
    "ПРОГНОЗ РОСТА": "TrendingUp",
  };

  return (
    <div style={{
      background: "linear-gradient(135deg, #0d1a10 0%, #0f2518 100%)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        <div style={{
          ...
          <Icon name="Sparkles" size={18} style={{ color: G }} />
        </div>
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: 0.3 }}>AI-заключение</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", marginTop: 1 }}>Персональный финансовый разбор</div>
        </div>
      </div>
      ...
      {status === "loading" && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, color: "rgba(255,255,255,0.55)", fontSize: 13, padding: "8px 0" }}>
          ...
          Анализирую твою финансовую картину...
      ...
      {status === "done" && sections.map((sec, i) => (
        <div key={i} style={{ marginBottom: i < sections.length - 1 ? 20 : 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            />
            <span style={{ fontSize: 11, fontWeight: 800, color: G, textTransform: "uppercase", letterSpacing: 1.2 }}>
              {sec.title}
            </span>
          </div>
