...
function StaffCard({ emp }: { emp: StaffResult }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eee", overflow: "hidden" }}>
      <div onClick={() => setOpen(p => !p)} style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
        {/* Score */}
        <div style={{ width: 48, height: 48, borderRadius: 12, background: cat.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: cat.color, lineHeight: 1 }}>{emp.score}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginTop: 14 }}>
            {[
              ...
              <div key={label} style={{ background: "#f8f8f5", borderRadius: 10, padding: "10px 12px" }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color }}>{value.toLocaleString()} ₽</div>
              </div>
            ))}
          </div>
          {emp.potential > 0 && (
            <div style={{ marginTop: 10, padding: "10px 14px", background: "hsl(145,60%,96%)", borderRadius: 10 }}>
              <div style={{ fontSize: 12, color: "hsl(145,60%,35%)", fontWeight: 600 }}>
                <Icon name="TrendingUp" size={13} style={{ display: "inline", marginRight: 6 }} />
                Потенциал роста: +{emp.potential.toLocaleString()} ₽/мес при устранении проблем
              </div>
            </div>
          )}
        </div>
    ...
    <div style={{ maxWidth: 760 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[...staff].sort((a, b) => b.score - a.score).map((emp, i) => <StaffCard key={i} emp={emp} />)}
        </div>
      </div>
