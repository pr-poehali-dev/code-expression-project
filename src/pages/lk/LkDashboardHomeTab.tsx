...
export function HomeTab({ onNav, role, hasSalon }: HomeTabProps) {
  ...
  const quickItems = [
    ...
    ...((role === "owner" || role === "admin" || role === "solo_master") && hasSalon ? [
    ...
    ...((role === "owner" || role === "admin" || role === "solo_master") ? [
  ...
  return (
    <div>
      ...
      {hasSalon && (role === "owner" || role === "admin" || role === "solo_master") && (
      ...
      <div style={{ marginTop: 32, background: "#fff", borderRadius: 16, padding: "22px 24px", border: "1px solid #E8ECF0", boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            { date: "28 июля 2026", text: "Запущена «ИИ-примерка образа» — покажите клиенту результат стрижки, окрашивания, макияжа или маникюра ещё до начала работы. Меньше недопонимания в кресле, меньше переделок и негативных отзывов — и заметно выше доверие к вашей экспертности.", highlight: true },
            { date: "12 июня 2026", text: "Бесплатная диагностика салона — пройдите «Диагностику роста салона PRO» прямо сейчас и узнайте, где ваш салон теряет деньги. Энергия уже на балансе, платить ничего не нужно." },
