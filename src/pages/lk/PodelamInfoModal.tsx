...
export default function InfoModal({ onClose }: { onClose: () => void }) {
  const items: { icon: string; title: string; text: string }[] = [
    ...
    },
    {
      ...
      text: "На основе вашей диагностики (стоимость услуги, доход, база клиентов, свободное время) ИИ считает разрыв между текущим и желаемым доходом и раскладывает его на понятные точки роста — сколько клиентов вернуть, сколько времени заполнить, что предложить дополнительно.",
    },
    {
      icon: "RefreshCw",
    ...
    },
    {
      ...
      text: "В блоке «Доход за сегодня» впишите сумму и нажмите «Добавить» — можно вносить несколько раз за день по мере поступления оплат, суммы складываются. Эти данные попадают в статистику и показывают, насколько факт совпадает с потенциалом плана.",
    },
    {
      icon: "Info",
      title: "Важно",
