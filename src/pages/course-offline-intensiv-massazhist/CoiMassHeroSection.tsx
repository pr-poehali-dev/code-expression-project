...
import { ACCENT, HERO_IMG, h2style, BtnPay, BtnBook } from "./CoiMassShared";

export default function CoiMassHeroSection() {
  return (
    <>
      {/* ── 1. HERO ── */}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "60px 24px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center" }} className="coi-hero-grid">
          <div>
            ...
            <h1 style={{ fontFamily: "Cormorant, serif", fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 700, lineHeight: 1.15, margin: "0 0 20px" }}>
              Вы уже работаете руками. Пора научиться восстанавливать человека глубже
            </h1>
            <p style={{ fontSize: 17, color: "#555", lineHeight: 1.7, margin: "0 0 28px" }}>
              Интенсив для практикующих массажистов — освойте восстановительные техники, диагностику руками и научитесь работать с болью, зажимами и травмами
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 16, marginBottom: 36 }}>
              {[
                { icon: "Eye", text: "Видеть причину боли" },
                { icon: "HandHeart", text: "Восстановительные техники" },
                { icon: "Star", text: "Специалист, а не массажист" },
              ].map(({ icon, text }) => (
          ...
          <div style={{ borderRadius: 20, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            <img src={HERO_IMG} alt="Интенсив для практикующих массажистов" className="coi-hero-img" style={{ width: "100%", height: 460, objectFit: "cover", display: "block" }} />
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "80px 0 0" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "0 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="problems-grid">
            {[
              "К вам приходят с болью в шее и пояснице, а эффекта хватает ненадолго",
              "Не понимаете, почему одна и та же проблема возвращается снова",
              "Хотите работать с серьезными запросами, но не чувствуете уверенности",
              "Ощущение, что остановились в развитии и не растете как специалист",
              "Клиенты уходят к остеопатам и мануальным терапевтам за «более серьезной» помощью",
              "Хотите большего, но не знаете, как выйти на новый уровень",
            ].map((t) => (
              <div key={t} style={{
          ...
          <div style={{ marginTop: 36, background: `${ACCENT}10`, border: `1.5px solid ${ACCENT}30`, borderRadius: 16, padding: "24px 28px" }}>
            <p style={{ margin: 0, fontSize: 16, color: "#1a1a1a", lineHeight: 1.7, fontWeight: 500 }}>
              На интенсиве вы научитесь смотреть на тело иначе — понимать, почему появляется боль, находить источник напряжения и применять техники, которые дают клиенту лёгкость уже после первых сеансов.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "80px 0 0" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="results-grid">
            {[
              { icon: "Search", title: "Найдете причину дискомфорта", text: "Научитесь быстрее находить источник боли, а не просто работать по симптомам" },
              { icon: "HandHeart", title: "Освоите мягкие техники", text: "Восстановительные техники, которые можно сразу применять в работе" },
              { icon: "Activity", title: "Увидите тело как систему", text: "Поймете, как комплексно работать с шеей, плечами, поясницей и спиной" },
              { icon: "Target", title: "Дадите заметный результат", text: "Клиенты начнут чувствовать разницу уже после первых процедур" },
              { icon: "PlayCircle", title: "Онлайн-курсы в подарок", text: "Все онлайн-курсы в подарок — пересматривайте техники и внедряйте методики" },
              { icon: "TrendingUp", title: "Выйдете на новый уровень", text: "Станете не просто массажистом, а специалистом по восстановлению тела" },
            ].map(({ icon, title, text }) => (
