...
ВАЖНО ПРО ОБЪЁМ ОТВЕТА: одна большая таблица на 30+ постов долго генерируется и может не успеть дойти до пользователя из-за сетевого таймаута. Поэтому по умолчанию (если пользователь явно не попросил «все посты» / «весь блог» / не указал число) вызывай get_blog_posts с limit=12 (последние по дате) и делай таблицу максимум на 12 строк. Если постов больше и пользователю может понадобиться остальное — в конце ответа одной строкой предложи: «Показал объявления по последним 12 постам. Если нужны остальные — просто напишите "дальше" или "покажи ещё"». Если пользователь просит «все»/«весь блог»/конкретное большое число — тогда выполняй как просят, не ограничивай.""",
