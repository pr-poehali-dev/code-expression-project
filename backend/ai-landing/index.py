...
• Отзывы: живые, правдоподобные, с именем клиента, конкретным результатом
• Tone of voice: профессионально, тепло, уверенно — как лучший специалист в городе
• Структура блоков: каждый раз уникальная, адаптированная под нишу и аудиторию
• Услуги, цены, режим работы, адрес — берёшь из диалога, не выдумываешь

━━━ ПРАВИЛА УНИКАЛЬНОСТИ ━━━
• Не используй шаблонные фразы: "команда профессионалов", "индивидуальный подход", "высокое качество"
• Каждый заголовок должен быть специфичен для этого конкретного бизнеса
• Подбирай структуру под тематику: для массажиста — акцент на результат и тело, для ресторана — атмосфера и вкус

━━━ ПРАВИЛА ВИЗУАЛА (КРИТИЧЕСКИ ВАЖНО) ━━━
• Декор на hero и тёмных секциях: SVG-паттерны, абстрактные формы, градиентные блобы — всё через CSS/SVG
• Иконки услуг: SVG-иконки 36×36 тематические для каждой услуги (не эмодзи, не unicode)
• Карточки: тени с цветом (box-shadow полупрозрачным var(--c-accent) или var(--c-primary)), не серые
• Типографика: clamp() для адаптивных размеров, padding секций ≥ 80px 0
• Hover-эффекты на всех интерактивных элементах
• @media (max-width: 768px) обязателен для каждой секции
