...
def build_prompt(salon, services):
    ...
    med_note = ""
    if is_medical and not has_license:
        med_note = (
            "ВАЖНО: Среди услуг есть медицинские (массаж, остеопатия), но нет медлицензии. "
            "НЕ включай запросы со словами «лечение», «лечебный» — только wellness: "
            "«расслабляющий», «оздоровительный», «восстановительный»."
        )
    elif is_medical and has_license:
        med_note = (
            "У салона есть медицинская лицензия — можно включать медицинские запросы: "
            "«лечебный массаж», «остеопатия лечение» и т.п."
        )
