...
def handle_register(event: dict) -> dict:
    ...
    Независимый специалист (мастер, психолог, телесный психолог) может указать промокод
    школы-партнёра — разово начисляется бонусная энергия (см. _apply_partner_promo_code),
    для владельца салона (компании) промокод не действует.
    ...
    try:
        ...
        # Действует для независимых специалистов (мастер, психолог, телесный психолог) —
        # все они регистрируются как один и тот же "личный салон"-контейнер, но не для
        # владельца компании (salon).
        school = None
        if promo_code:
            if user_type not in ("solo_master", "psychologist", "body_psychologist"):
                return err("Промокод школы действует только при регистрации мастера или частного специалиста")
            cur.execute(
...
def handle_reel_ideas(event: dict) -> dict:
    ...
    duration = "10s" if body.get("duration") == "10s" else "5s"
    duration_label = "10 секунд" if duration == "10s" else "5 секунд"
    if not service:
    ...
    try:
        ...
        if salon:
            ...
            salon_ctx = "\n".join(parts)
        prompt = (
            f"Ты — SMM-специалист для салона красоты. Придумай 5 идей для очень короткого видео-ролика ровно на {duration_label} (это будет сгенерировано ИИ, поэтому идея должна укладываться в одну простую сцену).\n\n"
            f"Услуга/тема: {service}\n"
            + (f"Цель рилса: {goal}\n" if goal else "")
            + (f"Стиль: {tone}\n" if tone else "")
            + (f"Контекст салона:\n{salon_ctx}\n" if salon_ctx else "")
            + "\nКаждая идея — это 1 строка: цепляющий заголовок + в скобках краткая суть (что происходит в видео, максимум 1-2 действия).\n"
            "Примеры формата:\n"
            "Главная ошибка ухода за маникюром (крупный план, мастер показывает на модели)\n"
            "До/после: преображение за секунды (быстрый переход между двумя кадрами)\n\n"
...
def handle_reel_script(event: dict) -> dict:
    ...
    try:
        ...
        # Сценарий строго под длительность реальной ИИ-генерации видео (5 или 10 сек) —
        # чтобы не было расхождения между сценарием и итоговым роликом
        if duration == "10s":
            duration_label = "ровно 10 секунд"
            structure = """
🎬 КРЮЧОК (0–2 сек)
...
[Призыв к действию]"""
        ...
        prompt = (
            f"Напиши сценарий очень короткого видео-ролика длительностью {duration_label} для салона красоты. "
            f"Это будет сгенерировано ИИ-видео-сервисом, поэтому сценарий должен описывать ОДНУ простую сцену "
            f"без резких смен ракурсов и большого количества действий — ИИ не умеет монтировать несколько кадров подряд.\n\n"
            f"Идея: {idea}\n"
            + (f"Услуга/тема: {service}\n" if service else "")
            + (f"Цель: {goal}\n" if goal else "")
            + (f"Стиль: {tone}\n" if tone else "")
            + (f"Контекст салона:\n{salon_ctx}\n" if salon_ctx else "")
            + f"""
Структура сценария (строго уложись в {duration_label}, не выходи за рамки):
{structure}
...
Требования:
- Ровно {duration_label} — никаких дополнительных кадров сверх структуры
- Одна связная сцена, а не монтаж из нескольких разных мест/ракурсов
...
- На русском языке"""
        ...
        # ИИ-модель видео не умеет корректно рисовать русский текст на экране — вместо букв
        # получаются нечитаемые символы, поэтому явно просим обойтись без текста/надписей.
        # Также модель не говорит по-русски — люди в кадре не должны говорить и издавать звуки,
        ...
        video_prompt = (
            ...
            "Без текста на экране, без надписей, без субтитров, без вывесок с читаемыми словами — только визуальное действие. "
            "Люди в кадре не говорят и не издают звуков, без диалогов и закадрового голоса — только фоновая музыка."
        )
        return ok({"script": content, "image_prompt": image_prompt, "video_prompt": video_prompt, "duration": duration})
    finally:
        conn.close()
