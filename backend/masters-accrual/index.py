...
def handler(event: dict, context) -> dict:
    ...
    if route_action not in KNOWN_ACTIONS:
        # Источник найден и подтверждён логами: на этот URL ошибочно настроен Telegram
        # webhook (шлёт update_id/message при активности в группе Promt Dialog) — сам
        # проект тут ни при чём, вебхук нужно перенастроить на правильный URL/удалить
        # через Telegram Bot API (deleteWebhook), это не задача кода. Здесь просто отвечаем
        # мгновенно без обращения к БД, чтобы такие чужие запросы ничего не стоили.
