...
GET/POST ?action=content_daily_post&key=ADMIN_TOKEN — cron: ИИ пишет ежедневный экспертный пост и публикует в блог сайта
                                       ...
                                       owner → admin → master → massage → psych (см. CONTENT_ROLES/CONTENT_ROLE_GUIDANCE).
                                       Для роли psych (психолог/телесный психолог, частная практика) используется
                                       отдельный набор тем по всем категориям (см. CONTENT_TOPICS_PSYCH) и отдельная
                                       справка по инструментам (CONTENT_TOOLS_REFERENCE_PSYCH) — категория «Допродажи»
                                       для психолога переосмыслена как «доход и финансовая устойчивость практики»
                                       (ценообразование, форматы работы), без техник допродажи услуг — это прямо
                                       запрещено в CONTENT_ROLE_GUIDANCE["psych"], так как неприменимо и неэтично.
                                       Один
...
CONTENT_ROLE_ORDER = ["owner", "admin", "master", "massage", "psych"]
...
общих бьюти-процедур, и без медицинских диагнозов/обещаний лечения.""",
    "psych": """Пишешь для ПСИХОЛОГА / ТЕЛЕСНОГО ПСИХОЛОГА в частной практике — человека, который ведёт консультации \
один на один или с парами/семьями, а НЕ мастера бьюти-индустрии. Это меняет саму суть того, что можно советовать.

СТРОГО ЗАПРЕЩЕНО:
— Любые советы про допродажи, «продать» клиенту дополнительную услугу, апселл, кросс-продажи, скидки/акции ради \
роста чека. У психолога НЕТ «допуслуг» в том смысле, что у салона — есть только консультации, форматы работы \
(индивидуально/пара/группа) и, возможно, пакет из нескольких консультаций, который предлагается как ЗАБОТА о \
процессе клиента, а не коммерческая допродажа. Никогда не используй слова «допродажа», «апселл», «средний чек», \
«акция», «скидка», «впарить», «продать».
— Любые упоминания конкретного содержания консультаций, диагнозов, состояний клиентов, деталей случаев — даже \
обезличенно. Психолог связан профессиональной этикой и конфиденциальностью строже, чем любая другая роль на \
платформе. Разрешены только общие профессиональные темы: организация практики, поток обращений, коммуникация с \
обратившимся ДО начала работы, границы, профилактика выгорания, супервизия, повышение квалификации.
— Тон «продающего эксперта» или «инфобизнеса». Психология — это доверие и этика, а не воронка продаж. Пиши сдержанно, \
профессионально, без манипулятивных приёмов, без «болей» и «триггеров» в маркетинговом смысле.

Его волнует: как обратившийся находит его и решается на первый контакт, как провести первую консультацию так, чтобы \
человек понял формат и решил продолжать, как выстроить регулярность работы (клиент возвращается на консультации не \
потому что его «удержали», а потому что видит ценность процесса), как выстраивать личный бренд специалиста через \
экспертный контент (не через «псевдо-диагностику» подписчиков), как справляться с профессиональным выгоранием, как \
находить супервизию и профессиональное сообщество, как соблюдать этику в рекламе психологических услуг (не обещать \
результат, не давить на боль).

Раскрывай темы «Маркетинг» через: как рассказать о своей практике и специализации так, чтобы это вызывало доверие, \
а не отпугивало; контент, который объясняет, как проходит консультация и с чем можно/нельзя обратиться, без \
самодиагностики читателя.
Раскрывай тему «Допродажи» (для психолога это раздел ФИНАНСОВАЯ УСТОЙЧИВОСТЬ ПРАКТИКИ) через: ценообразование \
консультации, устойчивый график записи, предложение формата продолжения работы (не «продажа», а честный разговор о \
том, что процесс требует регулярности), финансовую дисциплину частнопрактикующего специалиста.
Раскрывай тему «Работа с клиентами» через: первый контакт и обратную связь до начала работы, объяснение сеттинга \
(правила, длительность, оплата, отмены), уважительное завершение работы, этичные ответы на отзывы (без деталей \
случая), а НЕ через «удержание» и «возврат» в коммерческом смысле.""",
}


def get_next_content_role(conn) -> str:
    """Определяет роль читателя для следующего поста по ротации owner → admin → master → massage → psych,
...
# ── Отдельный набор тем для роли psych (психолог/телесный психолог, частная практика) ──────
# У психолога в ЛК недоступны вкладки "ai"/"salon"/"employees"/"championship" (только owner/admin/
# master имеют к ним доступ) — поэтому темы категории "tools" ссылаются ТОЛЬКО на реально доступные
# психологу разделы: ПоДелам (home), Академия (academy), Развитие личности — тесты мышления/барьеров/
# финансов (tools), портрет аудитории/офферы/посты/изображения/семантика (marketing), сообщения
# клиентам (clientmsg). Категория "upsell" переосмыслена как «Доход и устойчивость практики» — без
# единого слова о допродажах, только ценообразование и организация практики (см. CONTENT_ROLE_GUIDANCE).
CONTENT_TOPICS_PSYCH = {
    "marketing": [
        "как рассказать о своей практике так, чтобы вызвать доверие, а не показаться навязчивым",
        "как объяснить в соцсетях, с чем можно обратиться, не скатываясь в самодиагностику для читателя",
        "личный бренд психолога: как выбрать тему для блога и не выгореть на регулярном контенте",
        "как оформить страницу или сайт частной практики, чтобы обратившийся понял формат работы",
        "этичная реклама психологических услуг: что можно обещать, а что нет",
        "как выбрать канал продвижения психолога — без бюджета и с ним",
        "почему конкретика в описании специализации привлекает больше подходящих клиентов, чем общие фразы",
    ],
    "upsell": [
        "как выставить стоимость консультации и не занижать её из чувства неловкости",
        "разовая консультация или пакет: как честно предложить клиенту продолжение работы",
        "финансовая дисциплина частнопрактикующего психолога: учёт дохода и загрузки расписания",
        "как заполнить свободные часы записи, не жертвуя качеством консультаций",
        "работа с парой, семьёй или группой: как это влияет на доход практики",
        "супервизия и интервизия: как регулярные вложения в себя окупаются в практике",
    ],
    "clients": [
        "как провести первый контакт с обратившимся до начала работы",
        "как объяснить клиенту сеттинг: длительность, оплату, отмены и правила связи",
        "что делать, если клиент пропадает после первой консультации",
        "как этично попросить обратную связь или отзыв, не раскрывая деталей работы",
        "как завершать работу с клиентом уважительно, когда цели достигнуты",
        "профессиональные границы в переписке с клиентом вне сессий",
        "как справляться с профессиональным выгоранием и не терять качество консультаций",
    ],
    "tools": [
        "ПоДелам: обзор навигатора дохода — как ИИ строит план на день для частной практики",
        "ПоДелам: как навигатор учитывает специфику психолога, а не только салонов красоты",
        "Академия: какие курсы полезны психологу для развития практики, а не только для мастеров",
        "Портрет целевой аудитории: как понять, кому и с каким запросом вы можете быть полезны",
        "Генератор постов для соцсетей: как быстро получить черновик экспертного поста психолога",
        "Генерация изображений: как оформить визуал для поста без нарушения приватности клиентов",
        "Диагностика мышления и внутренних барьеров: зачем психологу проходить их самому",
        "Сообщения клиентам: как напомнить о записи или предложить продолжение работы корректно",
    ],
}

CONTENT_TOOLS_TOPIC_LINKS_PSYCH: dict[str, dict] = {
    "ПоДелам: обзор навигатора дохода — как ИИ строит план на день для частной практики": {
        "label": "Открыть навигатор «ПоДелам»", "desc": "Персональный план дел на сегодня уже ждёт в кабинете",
        "icon": "Compass", "tab": "home", "tool": None,
    },
    "ПоДелам: как навигатор учитывает специфику психолога, а не только салонов красоты": {
        "label": "Открыть навигатор «ПоДелам»", "desc": "План строится с учётом вашей специализации",
        "icon": "Compass", "tab": "home", "tool": None,
    },
    "Академия: какие курсы полезны психологу для развития практики, а не только для мастеров": {
        "label": "Перейти в Академию", "desc": "Курсы для развития практики и профессиональных навыков",
        "icon": "GraduationCap", "tab": "academy", "tool": None,
    },
    "Портрет целевой аудитории: как понять, кому и с каким запросом вы можете быть полезны": {
        "label": "Открыть портрет аудитории", "desc": "Понять, кому и с каким запросом вы можете быть полезны",
        "icon": "Users", "tab": "marketing", "tool": "audience",
    },
    "Генератор постов для соцсетей: как быстро получить черновик экспертного поста психолога": {
        "label": "Открыть генератор постов", "desc": "Тема → заголовки → готовый текст за пару минут",
        "icon": "FileText", "tab": "marketing", "tool": "post-gen",
    },
    "Генерация изображений: как оформить визуал для поста без нарушения приватности клиентов": {
        "label": "Открыть генерацию изображений", "desc": "Визуалы под пост без реальных фото клиентов",
        "icon": "Image", "tab": "marketing", "tool": "image-gen",
    },
    "Диагностика мышления и внутренних барьеров: зачем психологу проходить их самому": {
        "label": "Пройти диагностику", "desc": "Тесты мышления и внутренних барьеров с рекомендациями",
        "icon": "Brain", "tab": "tools", "tool": None,
    },
    "Сообщения клиентам: как напомнить о записи или предложить продолжение работы корректно": {
        "label": "Открыть генератор сообщений", "desc": "Корректные формулировки для переписки с клиентами",
        "icon": "MessageSquare", "tab": "clientmsg", "tool": None,
    },
}

CONTENT_TOOLS_REFERENCE_PSYCH = """СПРАВКА ПО ИНСТРУМЕНТАМ ПЛАТФОРМЫ «ПРОМТ ДИАЛОГ», ДОСТУПНЫМ ПСИХОЛОГУ В ЧАСТНОЙ \
ПРАКТИКЕ (используй ТОЛЬКО эти факты, ничего не выдумывай — психологу НЕ доступны разделы «Мой салон», «Развитие \
салона» (аудиты, скрипты администратора, ответы на отзывы), «Сотрудники» и «Чемпионат» — они предназначены только \
владельцам/администраторам салонов):

— ПоДелам (навигатор дохода) — стартовая страница личного кабинета. ИИ анализирует доход, стоимость консультации и \
поток обращений, строит персональный план дел на день с конкретными задачами, показывает прогресс к цели месяца.

— Академия — курсы и уроки внутри личного кабинета с модулями и домашними заданиями, можно применить знание сразу \
на практике и задать вопрос по уроку, не уходя со страницы обучения.

— Портрет целевой аудитории и офферы — ИИ помогает описать, с каким запросом и какому человеку специалист может быть \
полезен, чтобы точнее формулировать, о чём писать и как себя позиционировать.

— Генератор постов для соцсетей — тема → заголовки → готовый текст поста.

— Генерация изображений — визуалы под пост, без использования реальных фото клиентов.

— Сообщения клиентам — генератор персональных сообщений (напоминание о записи, предложение продолжения работы, \
поздравление) с корректными, неманипулятивными формулировками.

— Диагностика мышления и внутренних барьеров — тесты для самого специалиста, показывают личные ограничения, \
мешающие росту практики, с рекомендациями.

— ИИ-агент — чат с ИИ-ассистентом внутри навигатора «ПоДелам», отвечает на вопросы по развитию практики.
"""
...
def call_content_ai(topic: str, category: str, role: str) -> dict | None:
    ...
    мастер/массажист/психолог). Для роли psych используется отдельная справка по инструментам
    (CONTENT_TOOLS_REFERENCE_PSYCH) и другое вступление — без бьюти-контекста. Возвращает None
    при ошибке."""
    api_key = os.environ.get("POLZA_AI_API_KEY", "")
    if not api_key:
        return None
    ...
    if category == "tools":
        tools_reference_block = f"\n{CONTENT_TOOLS_REFERENCE_PSYCH if is_psych else CONTENT_TOOLS_REFERENCE}\n"
    else:
        tools_reference_block = ""

    intro_line = (
        "Ты — практикующий эксперт по частной психологической практике, ведёшь блог «Промт Диалог» (платформа для "
        "частнопрактикующих психологов, специалистов и салонов красоты: маркетинг, обучение, ИИ-инструменты, "
        "навигатор дохода «ПоДелам»)."
        if is_psych else
        "Ты — практикующий эксперт по бизнесу в бьюти-индустрии, ведёшь блог «Промт Диалог» (платформа для салонов "
        "красоты и мастеров: маркетинг, обучение, ИИ-инструменты, навигатор дохода «ПоДелам»)."
    )
    category_desc_upsell = (
        "Доход и устойчивость практики — ценообразование консультации, организация расписания, финансовая "
        "дисциплина частной практики. НИКАКИХ допродаж, апселла, скидок и акций — это неприменимо и неэтично "
        "для психолога, см. подробности в разделе про роль ниже."
        if is_psych else
        "Допродажи — рост среднего чека, допуслуги, абонементы, пакеты услуг, финансовая аналитика мастера/салона."
    )
    hashtag_examples = (
        "например #психологияотношений #частнаяпрактика #психологонлайн"
        if is_psych else
        "например #салонкрасоты #владелецсалона #маркетингдлясалона"
    )
    practitioner_desc = (
        "сильный практикующий психолог с опытом ведения частной практики"
        if is_psych else
        "сильный практик салонного бизнеса"
    )
    team_suffix = "" if is_psych else " или командой"
    upsell_category_guidance = (
        "Для роли psych эта рубрика называется «Доход и устойчивость практики»: пиши ТОЛЬКО про ценообразование "
        "консультации, организацию расписания, финансовую дисциплину частной практики, супервизию как инвестицию "
        "в качество работы. СТРОГО ЗАПРЕЩЕНО использовать слова «допродажа», «допуслуга», «апселл», «средний чек», "
        "«акция», «скидка», «впарить» — у психолога нет допродаж, есть только честный разговор о формате и "
        "регулярности работы."
        if is_psych else
        "Рассказывай, как экологично и профессионально предлагать дополнительные услуги, комплексы, абонементы, "
        "домашний уход, подарочные сертификаты и повторные визиты.\n"
        "Главный принцип: не навязывать и не давить, а подбирать решение под задачу клиента. Объясняй, как увеличить "
        "средний чек через заботу, диагностику потребностей, понятную рекомендацию и корректную коммуникацию."
    )

    system_prompt = f"""{intro_line}
...
- Допродажи — {category_desc_upsell}
...
- В самом конце добавь 4-6 релевантных хэштегов на русском по теме поста, роли и ниши ({hashtag_examples}) — без \
хэштегов на отвлечённые темы.
...
Пиши так, будто текст подготовил {practitioner_desc}, который сам работал в роли «{role_label}», ежедневно общается \
с клиентами{team_suffix} в этой роли, видит типичные ошибки и понимает экономику записи именно с этой позиции. \
Текст должен вызывать ощущение живого человеческого опыта, а не шаблонной статьи, созданной ИИ.
...
КАТЕГОРИИ И УГЛЫ РАСКРЫТИЯ (для роли psych — ниже общие ориентиры категорий, но ВСЕГДА в приоритете конкретные \
указания из блока про роль «{role_label}» выше: без допродаж, без деталей о клиентах, с акцентом на этику и \
конфиденциальность)
...
def handle_content_daily_post(event: dict, conn) -> dict:
    ...
    # Психолог (частная практика) не пользуется разделами салона — у него отдельный набор тем
    # по всем категориям и отдельная карта ссылок на инструменты (см. CONTENT_TOPICS_PSYCH /
    # CONTENT_TOOLS_TOPIC_LINKS_PSYCH), которые реально доступны в его личном кабинете.
    topics_pool = CONTENT_TOPICS_PSYCH if role == "psych" else CONTENT_TOPICS_BY_CATEGORY
    tool_links_pool = CONTENT_TOOLS_TOPIC_LINKS_PSYCH if role == "psych" else CONTENT_TOOLS_TOPIC_LINKS
    cur.execute(
    ...
    available_topics = [t for t in topics_pool[category] if t not in recent_topics]
    topic = random.choice(available_topics or topics_pool[category])
    ...
    # ссылка строго из справочника (свой набор для psych), ИИ её не придумывает.
    tool_link = tool_links_pool.get(topic) if category == "tools" else None
